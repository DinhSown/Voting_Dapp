...
function App() {
  ...
  // Admin modal states
  type AdminModal = "none" | "create-election" | "add-candidate" | "whitelist" | "elections-list";
  const [adminModal, setAdminModal] = useState<AdminModal>("none");
  const [elections, setElections] = useState<adminService.Election[]>([]);
  const [adminLoading, setAdminLoading] = useState(false);
  const [electionTitle, setElectionTitle] = useState("");
  const [candidateName, setCandidateName] = useState("");
  const [selectedElectionId, setSelectedElectionId] = useState<number | null>(null);
  const [whitelistInput, setWhitelistInput] = useState("");
  const [selectedElectionForWhitelist, setSelectedElectionForWhitelist] = useState<number | null>(null);
  ...
  const loadElections = async () => {
    try {
      ...
      pushLog("Tải danh sách cuộc bình chọn thành công");
    } catch (error: any) {
      notify(
        error.response?.data?.error || "Lỗi tải danh sách cuộc bình chọn",
        "error",
      );
    }
  };
  ...
  const handleCreateElection = async () => {
    if (!electionTitle.trim()) {
      return notify("Nhập tiêu đề cuộc bình chọn", "error");
    }
    try {
      setAdminLoading(true);
      await adminService.createElection(electionTitle);
      notify("Cuộc bình chọn đã được tạo thành công!", "success");
      pushLog(`✓ Tạo cuộc: ${electionTitle}`);
      setElectionTitle("");
      setAdminModal("none");
      loadElections();
    } catch (error: any) {
      notify(
        error.response?.data?.error || "Lỗi tạo cuộc bình chọn",
        "error",
      );
    } finally {
  ...
  const handleAddCandidate = async () => {
    if (!selectedElectionId) {
      return notify("Chọn cuộc bình chọn", "error");
    }
    if (!candidateName.trim()) {
      return notify("Nhập tên ứng viên", "error");
    }
    try {
      setAdminLoading(true);
      await adminService.addCandidate(selectedElectionId, candidateName);
      notify("Ứng viên đã được thêm thành công!", "success");
      pushLog(`✓ Thêm ứng viên: ${candidateName}`);
      setCandidateName("");
      setAdminModal("none");
      loadElections();
    } catch (error: any) {
      notify(
        error.response?.data?.error || "Lỗi thêm ứng viên",
        "error",
      );
    } finally {
      setAdminLoading(false);
  ...
  const handleWhitelistWallets = async () => {
    if (!selectedElectionForWhitelist) {
      return notify("Chọn cuộc bình chọn", "error");
    }
    const wallets = whitelistInput
      .split("\n")
      .map((w) => w.trim())
      .filter((w) => w.startsWith("0x"));
    if (wallets.length === 0) {
      return notify("Nhập địa chỉ ví hợp lệ (mỗi dòng một địa chỉ)", "error");
    }
    try {
      setAdminLoading(true);
      await adminService.whitelistWallets(
        selectedElectionForWhitelist,
        wallets,
      );
      notify(`${wallets.length} ví đã được cấp quyền!`, "success");
      pushLog(`✓ Cấp quyền từng ${wallets.length} ví`);
      setWhitelistInput("");
      setAdminModal("none");
    } catch (error: any) {
      notify(
        error.response?.data?.error || "Lỗi cấp quyền ví",
        "error",
      );
    } finally {
      setAdminLoading(false);
  ...
  const handleStartElection = async (electionId: number) => {
    try {
      setAdminLoading(true);
      await adminService.startElection(electionId);
      notify("Cuộc bình chọn đã được bắt đầu!", "success");
      pushLog(`▶ Bắt đầu cuộc bình chọn #${electionId}`);
      loadElections();
    } catch (error: any) {
      notify(
        error.response?.data?.error || "Lỗi bắt đầu cuộc bình chọn",
        "error",
      );
    } finally {
      setAdminLoading(false);
  ...
  const handleEndElection = async (electionId: number) => {
    try {
      setAdminLoading(true);
      await adminService.endElection(electionId);
      notify("Cuộc bình chọn đã kết thúc!", "success");
      pushLog(`■ Kết thúc cuộc bình chọn #${electionId}`);
      loadElections();
    } catch (error: any) {
      notify(
        error.response?.data?.error || "Lỗi kết thúc cuộc bình chọn",
        "error",
      );
    } finally {
      setAdminLoading(false);
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-5 md:px-8">
          ...
          <nav className="hidden gap-2 md:flex">
            {(["home", "vote", "results", "admin"] as Route[]).map((item) => (
              <button
                key={item}
                onClick={() => go(item)}
                className={`rounded-full px-4 py-2 font-label text-[11px] uppercase tracking-[0.22em] cursor-pointer transition-all duration-300 ${route === item ? "bg-primary text-on-primary drop-shadow-[0_0_10px_rgba(242,202,80,0.6)]" : "text-on-surface/65 hover:text-primary hover:drop-shadow-[0_0_8px_rgba(242,202,80,0.8)]"}`}
              >
                {item === "home"
                  ? "Trang chủ"
                  : item === "vote"
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="btn-glow rounded-full bg-primary px-5 py-3 font-label text-[11px] font-bold uppercase tracking-[0.22em] text-on-primary cursor-pointer disabled:cursor-not-allowed disabled:opacity-70 hover:scale-[1.02]"
            >
      ...
      <main className="relative z-10">
          ...
          <section className="mx-auto grid max-w-7xl gap-8 px-5 py-12 md:px-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="space-y-8 reveal">
              ...
              <div className="flex flex-wrap gap-4">
                <button
                  onClick={() => go("vote")}
                  className="btn-glow inline-flex items-center gap-3 rounded-full bg-primary px-7 py-4 font-label text-xs font-bold uppercase tracking-[0.24em] text-on-primary cursor-pointer hover:scale-[1.02]"
                >
            ...
            <div className="space-y-6 reveal">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                {categories.map((category) => (
                  <article key={category.id} className="panel group hover:border-primary/40 hover:shadow-[0_0_30px_rgba(242,202,80,0.15)] transition-all duration-500">
                    ...
                    <div className="mt-6 flex gap-3">
                      ...
                      <button
                        ...
                        className="btn-glow rounded-full bg-primary px-4 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-on-primary disabled:border disabled:border-white/10 disabled:bg-transparent disabled:text-on-surface/35 cursor-pointer disabled:cursor-not-allowed hover:scale-[1.02]"
                      >
                ...
                <div className="mt-5 space-y-4">
                  <div className="rounded-[1.6rem] border border-white/8 bg-black/18 p-5">
                    ...
                    <input
                      ...
                      className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-base outline-none input-glow transition-all cursor-pointer"
                      placeholder="your.email@example.com"
                    />
                    <button
                      onClick={sendOtp}
                      disabled={busy || !email}
                      className="mt-5 rounded-full border border-primary/40 bg-primary/10 px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-primary disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed hover:bg-primary/20 hover:shadow-[0_0_15px_rgba(242,202,80,0.2)] transition-all"
                    >
                      Gửi mã OTP
                    </button>
                  </div>
                  <div className="rounded-[1.6rem] border border-white/8 bg-black/18 p-5">
                    ...
                    <input
                      ...
                      maxLength={6}
                      className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-base tracking-[0.4em] text-primary outline-none input-glow transition-all cursor-pointer placeholder:tracking-normal"
                      placeholder="000000"
                    />
                    <div className="mt-5 flex gap-3">
                      <button
                        ...
                        className="btn-glow rounded-full bg-primary px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-on-primary disabled:opacity-40 cursor-pointer disabled:cursor-not-allowed hover:scale-[1.02]"
                      >
                        Xác minh
                      </button>
                      <button
        ...
        {route === "admin" && (
          <section className="mx-auto max-w-7xl px-5 py-12 md:px-8">
              ...
              <div className="space-y-6">
                ...
                <div className="panel reveal">
                  ...
                  <div className="mt-5 grid gap-3 grid-cols-1 md:grid-cols-2">
                    <button
                      onClick={() => setAdminModal("create-election")}
                      className="inline-flex items-center justify-center gap-2 rounded-[1.4rem] border border-primary/30 bg-primary/10 px-4 py-4 text-sm font-semibold leading-7 text-on-surface/80 cursor-pointer hover:bg-primary/20 transition"
                    >
                      <Plus size={16} /> Tạo cuộc
                    </button>
                    <button
                      onClick={() => { loadElections(); setAdminModal("elections-list"); }}
                      className="inline-flex items-center justify-center gap-2 rounded-[1.4rem] border border-primary/30 bg-primary/10 px-4 py-4 text-sm font-semibold leading-7 text-on-surface/80 cursor-pointer hover:bg-primary/20 transition"
                    >
                      <List size={16} /> Danh sách
                    </button>
                    <button
                      onClick={() => setAdminModal("add-candidate")}
                      className="inline-flex items-center justify-center gap-2 rounded-[1.4rem] border border-primary/30 bg-primary/10 px-4 py-4 text-sm font-semibold leading-7 text-on-surface/80 cursor-pointer hover:bg-primary/20 transition"
                    >
                      <Users size={16} /> Thêm ứng viên
                    </button>
                    <button
                      onClick={() => setAdminModal("whitelist")}
                      className="inline-flex items-center justify-center gap-2 rounded-[1.4rem] border border-primary/30 bg-primary/10 px-4 py-4 text-sm font-semibold leading-7 text-on-surface/80 cursor-pointer hover:bg-primary/20 transition"
                    >
                      <Key size={16} /> Cấp quyền
                    </button>
                  </div>
                </div>
              </div>
            ...
            {adminModal === "create-election" && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="w-full max-w-md rounded-2xl border border-white/10 bg-surface-container-high p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-headline text-2xl">Tạo cuộc bình chọn</h2>
                    <button
                      onClick={() => setAdminModal("none")}
                  ...
                  <div className="space-y-4">
                    <input
                      type="text"
                      placeholder="Tiêu đề cuộc bình chọn"
                      value={electionTitle}
                      onChange={(e) => setElectionTitle(e.target.value)}
                      className="w-full rounded-lg border border-white/10 bg-black/30 px-4 py-3 text-white placeholder-on-surface/50 focus:outline-none focus:border-primary"
                    />
                    <div className="flex gap-3">
                      <button
                        onClick={handleCreateElection}
                        disabled={adminLoading}
                        className="flex-1 rounded-lg bg-primary px-4 py-3 text-on-primary font-semibold hover:bg-primary/90 disabled:opacity-50"
                      >
                        {adminLoading ? "Đang xử lý..." : "Tạo"}
                      </button>
                      <button
                        onClick={() => setAdminModal("none")}
                        className="flex-1 rounded-lg border border-white/10 px-4 py-3 text-on-surface hover:bg-white/5"
                      >
                        Hủy
                      </button>
                    </div>
            ...
            {/* ============ MODAL: ADD CANDIDATE ============ */}
            {adminModal === "add-candidate" && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="w-full max-w-md rounded-2xl border border-white/10 bg-surface-container-high p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-headline text-2xl">Thêm ứng viên</h2>
                    <button
                      onClick={() => setAdminModal("none")}
                      className="text-on-surface/60 hover:text-on-surface"
                    >
                  ...
                  <div className="space-y-4">
                    <select
                      ...
                      className="w-full rounded-lg border border-white/10 bg-black/30 px-4 py-3 text-white focus:outline-none focus:border-primary"
                    >
                      <option value="">Chọn cuộc bình chọn</option>
            ...
            {/* ============ MODAL: WHITELIST WALLETS ============ */}
            {adminModal === "whitelist" && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="w-full max-w-md rounded-2xl border border-white/10 bg-surface-container-high p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-headline text-2xl">Cấp quyền bỏ phiếu</h2>
                    <button
                      onClick={() => setAdminModal("none")}
                      className="text-on-surface/60 hover:text-on-surface"
                    >
                  ...
                  <div className="space-y-4">
                    <select
                      value={selectedElectionForWhitelist || ""}
                      onChange={(e) => setSelectedElectionForWhitelist(Number(e.target.value) || null)}
                      className="w-full rounded-lg border border-white/10 bg-black/30 px-4 py-3 text-white focus:outline-none focus:border-primary"
                    >
                      <option value="">Chọn cuộc bình chọn</option>
                      {elections.map((election) => (
                    ...
                    <textarea
                      placeholder="Nhập địa chỉ ví (mỗi dòng một ví)&#10;Ví dụ:&#10;0x742d35Cc6634C0532925a3b844Bc9e7595f..."
                      value={whitelistInput}
                      onChange={(e) => setWhitelistInput(e.target.value)}
                      className="w-full h-32 rounded-lg border border-white/10 bg-black/30 px-4 py-3 text-white placeholder-on-surface/50 focus:outline-none focus:border-primary resize-none"
                    />
                    <div className="flex gap-3">
                      <button
                        onClick={handleWhitelistWallets}
                        disabled={adminLoading}
                        className="flex-1 rounded-lg bg-primary px-4 py-3 text-on-primary font-semibold hover:bg-primary/90 disabled:opacity-50"
                      >
                        {adminLoading ? "Đang xử lý..." : "Cấp quyền"}
                      </button>
                      <button
                        onClick={() => setAdminModal("none")}
                        className="flex-1 rounded-lg border border-white/10 px-4 py-3 text-on-surface hover:bg-white/5"
            ...
            {/* ============ MODAL: ELECTIONS LIST ============ */}
            {adminModal === "elections-list" && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-surface-container-high p-6 max-h-96 overflow-y-auto">
                  <div className="flex items-center justify-between mb-4 sticky top-0">
                    <h2 className="font-headline text-2xl">Danh sách cuộc bình chọn</h2>
                    <button
                      onClick={() => setAdminModal("none")}
                      className="text-on-surface/60 hover:text-on-surface"
                    >
                  ...
                  <div className="space-y-3">
                    {elections.length === 0 ? (
                      <p className="text-on-surface/60">Không có cuộc bình chọn nào</p>
                    ) : (
                      elections.map((election) => (
                        <div
                          key={election.id}
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <h3 className="font-semibold">#{election.id} - {election.title}</h3>
                              <p className="text-sm text-on-surface/60 mt-1">
                                Ứng viên: {election.candidateCount} | Phiếu: {election.totalVotes}
                              </p>
                            </div>
                            <span
                            ...
                            >
                              {election.isActive ? "✓ Đang diễn ra" : "○ Chưa bắt đầu"}
                            </span>
                          </div>
                          <div className="flex gap-2">
                            {!election.isActive && (
                              <button
                                onClick={() => {
                                  handleStartElection(election.id);
                                  setAdminModal("none");
                                }}
                                className="flex-1 text-xs rounded-lg bg-primary/20 text-primary px-3 py-2 hover:bg-primary/30"
                              >
                                ▶ Bắt đầu
                              </button>
                            )}
                            {election.isActive && (
                              <button
                                ...
                                className="flex-1 text-xs rounded-lg bg-red-500/20 text-red-400 px-3 py-2 hover:bg-red-500/30"
                              >
                                ■ Kết thúc
                              </button>
                            )}
